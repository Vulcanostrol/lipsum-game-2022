...
//        applyAugmentation(augmenter);
        ...
        if (this.bulletAmount == 1) {
            new Bullet(originX, originY, vector2.x * this.bulletSpeed, vector2.y * this.bulletSpeed, this.damage, bulletSize, this.explosionSize);
        } else {
            for (int i = 0; i < bulletAmount; i++) {
                ...
                new Bullet(originX, originY, (vector2.x + xOffset) * this.bulletSpeed, (vector2.y + yOffset) * this.bulletSpeed, this.damage, bulletSize, this.explosionSize);
            }
        }
        return true;
    }
    ...
    @Override
    public void augmentExplosion(int init, int add) {
        if (explosionSize == 0) {
            explosionSize = init;
        } else {
            explosionSize += add;
